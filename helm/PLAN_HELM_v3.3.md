...
Составлен 27.08.2026. Метод — ultraplan.
**Статус: офлайн-сборка идёт; живые фазы блокированы отсутствием SSH.**
Решения владельца от 27.08: собирать офлайн здесь · design brief прислан ·
цель первого прогона — до P12.
...
| ~~**B5**~~ | `HELM_PANEL_DESIGN_BRIEF.md` | — | **закрыт 27.08: файл прислан, SHA256 совпал точно** |
...
| ~~R5~~ | Нет design brief | — | — | **Снят: brief получен, цепочка §10.5.1 восстановлена полностью** |
...
## 9. Что уже собрано офлайн (обновлено 27.08.2026)

Дерево `helm/tree/` отображается на `/opt/helm` один в один. Всё проверено
запуском, а не чтением: настоящий PostgreSQL 16, настоящий Chromium.

| Готово | Фаза | Доказательство |
|---|---|---|
| Ядро Control Plane: 16 таблиц, policy, реестр действий, approvals, дедупликация, outbox | P2 | 49 тестов, из них §30.2 целиком по строке на тест |
| RED-гейт | A-DoD п.5–6 | блокировка без одобрения и ровно-однократность доказаны |
| Миграции Alembic | P2 | применение → сверка с моделями → откат → повторное применение |
| API: internal под HMAC, panel API, заголовки §30.7 | P2 | 11 тестов, включая подпись тела и запрет LLM в GET |
| Guardian + systemd + автоочистка §25.6 | P5 | §30.10 доказан: жив при мёртвых Docker/PG/CP/Hermes |
| Периметр: compose + Caddyfile | P1 | 11 тестов §4.6; мутации ловятся |
| Панель: 5 разделов, карточка одобрения, step-up | P7.5 | сборка + 27 проверок брифа + 3 вьюпорта |
| `checkpoint.sh` | P1 | цикл создание→список→сверка; секрет не утекает |
| ADR-009, ADR-017, ADR-018 | — | по фактически принятым решениям |

**Найдено и исправлено при проверке** (каждое — запуском, не чтением):

1. Миграция была сгенерирована против БД с уже созданными таблицами и вышла
   почти пустой. Перегенерирована против чистой базы, цикл проверен целиком.
2. «Сегодня» не умещалось на 390×844 на 35px — нарушение §10.5.11. Ужаты
   отступы блоков; высота строк из брифа §5 не тронута.
3. «Диск 41 %» вместо «41 / 100 GB» — прямое нарушение запрета брифа §2 на
   процент без абсолюта, которое статическая проверка пропускала. Проверка
   дополнена и сама проверена подсадкой нарушения.
4. `checkpoint.sh` глушил ошибку `tar` и создавал пустой архив. Молчаливо
   пустой чекпоинт хуже отсутствующего — теперь падает громко и проверяет,
   что каждый отслеживаемый путь реально попал в архив.
5. Статус предусловия в панели передавался только цветом и значком —
   добавлен текст для скринридера.

**Осталось до P12 и требует живого сервера или недостающих секретов:**
P0 preflight · P1 хост · P3 LiteLLM (нужен B2) · P4 Hermes и Telegram
(нужен B3) · P5 restic и restore-тест (нужен B4) · P6 n8n · P6.5 Forgejo ·
P7 MAX и MCP · P7.5 активация auth (нужен B8) · P8 dev lane · P9–P12 домены.
