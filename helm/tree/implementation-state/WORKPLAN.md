...
подтверждение «пригласить» подошло бы к «приостановить». Knowledge-only
оболочка для KNOWLEDGE_USER НЕ реализована: у secondary-пользователя нет
способа войти в панель вовсе, а выбор пути enrollment'а — решение
владельца в области безопасности (см. V3.8-DELTA.md).
...
297 → 304 → 334 → 350 → 370 → 386 → 388 тестов зелёных по ходу коммитов
этой фазы.
...
KNOWLEDGE_USER, Knowledge-оболочка панели для KNOWLEDGE_USER
(остаток P8.6.5),
