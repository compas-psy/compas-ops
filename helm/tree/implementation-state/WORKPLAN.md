...
### P8.5.7 — вложения Telegram/MAX: MAX-сторона готова офлайн, деплой не выполнен; Telegram открыт
...
вложения — `tests/test_max_channel.py`), 191 всего зелёных (было 158),
все 47 ранее существовавших тестов канала MAX не тронуты.

**Честно не проверено**: сама доставка (`docker-compose.yml`: `helm-core`
получил `group_add: ["1001"]` + volume mount'ы `/opt/helm-knowledge` и
`/opt/helm-state/knowledge-spool`; `knowledge-bootstrap.sh`: spool
`700`→`770`+setgid) не проверена на живом контейнере — тот же класс
проверки, что понадобился P8.5.2. Форма `message.body.attachments` у
реального MAX-вебхука тоже не подтверждена (`dev.max.ru` недоступен из
egress-политики песочницы разработки) — `parse_attachment()` написан по
документированному поведению TamTam-производных API; расхождение не
роняет вебхук (`MaxAttachmentUnsupported` перехватывается, лог несёт
имена полей payload, не значения, владелец получает нейтральное «не
смог скачать»). `scripts/knowledge-attachment-deploy-runbook.md` готов
для первой живой проверки.

**Telegram не реализовано.** `hermes/plugins/helm-control/__init__.py`
— синхронный колбэк внутри процесса Hermes gateway (не Control Plane,
не Docker) — неизвестно без чтения реального исходника, несёт ли
`event` что-то про вложение и доступен ли токен бота для его скачивания
изнутри плагина. Спека сама явно допускает этот исход ("smallest
transport adapter... if current stable Hermes plugin hook cannot
expose/download attachment safely"). Read-only разведка —
`scripts/knowledge-telegram-attachment-recon.sh`; решение и код —
после неё, не раньше (`ADR-021`).
