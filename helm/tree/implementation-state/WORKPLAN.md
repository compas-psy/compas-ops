...
## Текущая фаза: **Milestone A закрыт целиком.** Milestone B: Panel auth **закрыт**, n8n (P6) и Forgejo (P6.5) **подняты и подтверждены живьём**. Автономный заход 29.08.2026: **миграционный пакет Forgejo готов** (`scripts/forgejo-migrate.py` + ранбук, §18.3 шаги 1-9), **сторона Control Plane для MAX написана целиком** (§10.1/§10.3/§10.4 + доставщик outbox + `/force`; ADR-020; 117 тестов зелёные), F-260829-04 закрыт. Оба следующих шага упираются в сервер: миграция репозиториев и разведка Hermes для плагина `max-bridge` — ранбуки в `scripts/`. Первый n8n-коннектор — за выбором владельца (§17.2 start set, любому нужны его credentials/OAuth).
...
Остался плагин Hermes `max-bridge` (единственное недостающее звено),
плюс диагностика доставки в `dispatch.py` (тип исключения и HTTP-код в
лог, без текста сообщения — без неё эта же цепочка отладки заняла бы
кругов вдвое больше).

## MAX (P7) — сторона Control Plane написана 29.08.2026; остался плагин Hermes

Дизайн — `docs/adr/ADR-020-max-bridge.md`. Реализовано оффлайн, без
сервера, 117 тестов зелёные:

- `POST /hooks/max` — §10.1 дословно: секрет `X-Max-Bot-Api-Secret` →
  сверка владельца → дедуп → регистрация задачи → вызов локального chief
  API. Порядок не декоративный: вердикт дедупа нужен ДО обращения к
  Hermes, иначе на схлопнутом дубле chief ответит второй раз.
- `POST /internal/outbound` — ответ chief в очередь, ровно однократно.
- `dispatch.py` — доставщик outbox, которого не существовало вовсе:
  `enqueue()` был, а разбирать очередь было некому. Backoff-ретраи,
  FAILED после лимита, доставленный текст стирается из БД (§10.2).
- `/force` (§10.4) — снимает cross-channel дедуп и намеренно НЕ снимает
  идемпотентность по `external_message_id`.
- Молчаливое схлопывание дубля — решение владельца 29.08.2026.

Найдено при реализации, оба — тестами, не чтением:

1. **MAX-id владельца ≠ Telegram-id** (F-260829-17). Вебхук отвергал бы
   самого владельца; «починка в лоб» (регистрация под MAX-id) молча
   сломала бы §10.4 — хэш намерения считается вместе с owner_id, и хэши
   одного вопроса из двух каналов разошлись бы навсегда. Введено
   отдельное `max_owner_id`: сверка по нему, регистрация — под
   канонической identity.
2. **Окно cross-channel дедупа расходилось со спекой** (F-260829-18):
   10 минут в коде против 2 минут в §10.4, без ADR и без теста на само
   значение. Исправлено, тест на границу окна добавлен.

Осталось (только на сервере): разведка 4 внутренних интерфейсов Hermes,
секреты MAX, плагин `max-bridge`, живая приёмка §30.5 — всё в
`scripts/max-bringup-runbook.md`.

Проверено вживую: на сервере НЕТ запущенного "Hermes chief API server",
который требует §10.2 ("Control Plane использует Hermes Responses API с
named conversation"). `sudo ss -tlnp` показал только postgres/helm-core/
sing-box — ни один порт, принадлежащий Hermes, не слушает локально. Grep по
исходникам Hermes (`/home/helm/.hermes/hermes-agent`) на "chief API"/
"Responses API"/"named conversation" нашёл только код, которым Hermes САМ
обращается к OpenAI/Codex как к ПРОВАЙДЕРУ модели (`agent/auxiliary_client.py`
и т.п.) — это другое значение термина "Responses API", не входящий API
Hermes для внешних вызовов.

Вывод: §10.2 описывает функциональность, которой физически не существует —
не деталь интеграции, а отдельная фича внутри Hermes (вероятно, новый
plugin по образцу `helm-control`, только в обратную сторону: локальный
листенер, принимающий текст и прогоняющий его через того же chief-агента,
что отвечает в Telegram). Решение владельца: отложить, спроектировать
отдельным заходом. Что уже готово со стороны Control Plane/MAX API и не
пропадёт: `IngestService` уже поддерживает cross-channel дедуп между
произвольными каналами (написан раньше, generic, не только для Telegram) —
`/hooks/max` сможет переиспользовать его без изменений; реальный API MAX
для исходящих сообщений найден (`POST /messages`, `Authorization`-заголовок
с токеном бота, JSON `{chat_id, text}`).
