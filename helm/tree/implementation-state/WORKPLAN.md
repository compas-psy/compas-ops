...
`knowledge_user_id`. Полный разбор — `docs/adr/ADR-029-dedicated-
...
**P8.5.12 recall**: до этого шага Micro-Memory писалась, но не читалась
ничем, кроме прямых тестов — обычный вопрос владельца искал только
`KnowledgeChunk`. `helm_core/knowledge/recall.py` + подключение в
`probe()`: память имеет абсолютный приоритет над документами (§14.12
"strong exact boost"), одиночное попадание возвращается дословно
(§14.14 "exact URL/identifier not modified" — проверено равенством
строк), истечение проверяется предикатом В МОМЕНТ ЗАПРОСА, а не
фоновой рутиной (§14.10 прямым текстом; рутины в кодовой базе нет
вовсе), исторический вопрос («Какой был номер вчера?») дополнительно
видит `EXPIRED`/`SUPERSEDED`, но никогда `DISABLED`. «Напомни завтра в
10 позвонить …» распознаётся как постановка напоминания и не съедает
вопрос к памяти — требуется совпадение трёх признаков сразу, потому
что каждый по отдельности есть и в законном recall'е.

**P8.6.5 owner-половина «Система → Пользователи»**: `GET /api/panel/v1/
users` + инвайт/suspend/reactivate/квота + блок «Пользователи» в разделе
«Система» панели — заменяет стенд-ин `POST /internal/knowledge/users/
invite`, который ADR-029 помечал как временный. §14.3 «no normal content
browser across users» выполнено тем, какие таблицы названы в коде: все
эндпоинты читают только реестр тенантов и ни одной tenant-scoped таблицы
с контентом. §10.5.8.1 расширен на действия без одобрения —
`StepUp.assert_scope()` привязывает passkey-церемонию к операции И к
целевому пользователю (`panel:users:suspend:<uuid>`), иначе
подтверждение «пригласить» подошло бы к «приостановить». Вход
KNOWLEDGE_USER в панель — ОТДЕЛЬНЫМ enrollment-токеном (решение
владельца 30.08.2026), без единой миграции: `owner_id` в
`panel_sessions`/`webauthn_credentials`/`panel_enrollment_tokens` уже
строка-принципал, secondary получает `ku:<uuid>`, owner-вход не изменён
ни строкой. Вход после регистрации — usernameless (discoverable
credential), и главное его свойство закреплено тестом: этот путь НЕ
может выдать сессию владельца, иначе он был бы обходом
Telegram-виджета. Разделение ролей — отказом сервера
(`require_owner_session` во всех 14 owner-эндпоинтах), а не тем, что
фронт не нарисовал пункт меню. Knowledge-оболочка берёт тенанта ИЗ
СЕССИИ — подставить чужой идентификатор физически нечем; владельцу она
заодно даёт первую версию P8.5.8.

**P8.6.6 per-user style — не реализуется, и это решение, а не пробел**:
§14.14 описывает стиль как ступень конвейера синтеза (evidence pack →
factual draft → style capsule → Ollama localizer → fidelity guard).
Такой ступени в кодовой базе нет вовсе: probe() отвечает чистой
экстракцией, локализатор нигде не подключён, а стилевой капсулы
владельца в репозитории физически нет — `prompts/` и `skills/` пусты.
Строить таблицу стилевых профилей под несуществующую ступень — ровно
то, что запрещает CLAUDE.md §2. Важное же из P8.6.6 («styles never
cross users», «paid AI forbidden for KNOWLEDGE_USER») уже выполнено по
конструкции и закреплено структурными тестами разбора импортов.

297 → 304 → 334 → 350 → 370 → 386 → 388 → 415 тестов зелёных по ходу
коммитов этой фазы.
...
заходом (см. V3.8-DELTA.md, ADR-029): файлы/ZIP/голос для
KNOWLEDGE_USER,
vault-экспорт и RED-delete (остаток P8.6.7), голосовой Remember/recall
(GigaAM нигде не подключён) — полный v3.8 acceptance ждёт этих фаз,
READY не объявлено.
...
`docs/adr/ADR-026-zip-batch-ingest.md`.
...
каждого канала — `docs/adr/ADR-102-knowledge-attachment-transport.md`
(темы «транспорт вложений» в списке §34 нет вовсе, поэтому документ
живёт в собственном диапазоне 100+, см. `docs/adr/README.md`).
...
сосуществовать с ним — зафиксировать в `ADR-102` при реализации.
...
ним. Полный дизайн — `docs/adr/ADR-102-knowledge-attachment-
transport.md`.
...
ADR-014 с планом «новый плагин `max-bridge`, регистрация платформы
...
`docs/adr/ADR-014-max-responses-api.md` (переименован с `-max-bridge`,
...
- [x] `guardian.py`/`cleanup.sh` развёрнуты в `/opt/helm/guardian/`,
      ...
      ПОПРАВКА 07.09.2026. «Развёрнут» относилось только к `guardian.py`.
      `cleanup.sh` лежал рядом, но таймера у него не было, и за тридцать
      дней он не запустился ни разу (прогон 452: ноль упоминаний в
      журнале, `helm-cleanup.timer could not be found`). Автоочистка
      §25.6 не работала с самого начала; цена — 41.29 ГБ кэша сборки и
      диск на 84% к 07.09. Закрыто: `helm-cleanup.service`+`.timer`
      (каждый час) включены и проверены живым запуском (прогон 454,
      кэш 4.712 ГБ → 3.841 ГБ, `Result=success`); флаг `--keep-storage`,
      которого нет в docker 29.x, заменён; провал шага теперь виден в
      журнале и не обрывает уборку; `cleanup.sh` доставляется выкатом.
