...
## Текущая фаза: **Milestone A закрыт целиком.** Milestone B: Panel auth **закрыт**, n8n (P6) и Forgejo (P6.5) **подняты и подтверждены живьём**. Автономный заход 29.08.2026: **миграционный пакет Forgejo готов** (`scripts/forgejo-migrate.py` + `scripts/forgejo-migrate-runbook.md`, §18.3 шаги 1-9 — ждёт серверного батча владельца), **MAX спроектирован** (`docs/adr/ADR-020-max-bridge.md` — плагин max-bridge как фейковый adapter в gateway, ждёт разведки на сервере и решения по дедуп-коллапсу), F-260829-04 закрыт (74 теста зелёные). Первый n8n-коннектор — за выбором владельца (§17.2 start set, любому нужны его credentials/OAuth).
...
## MAX (P7) — спроектирован 29.08.2026: ADR-020-max-bridge.md (реализация — следующий батч)
