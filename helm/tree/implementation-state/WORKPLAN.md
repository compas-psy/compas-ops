...
**Честно не проверено**: сам `TelegramSender` на реальном сервере (нужен
повторный деплой `helm-core` + повторная отправка файла) и то, что чиф
на живом Telegram-сообщении с вложением реально НЕ вызывается напрямую
(решение владельца — критическая проверка, ещё не выполнена).
...
**Открытый вопрос — блокировка/удаление знаний (не реализовано, требует
решения владельца).** `KnowledgeStatus.ARCHIVED` уже существует и уже
исключён из `probe()` (значит "мягкая блокировка" — просто выставить
статус — уже сработает), но НИЧЕГО в кодовой базе его не выставляет.
Полное удаление (RAW-файл + SOURCE `.md` + chunks + строки БД) не
реализовано вовсе — есть только разовые ручные SQL-скрипты для смоук-
тестовых данных (`knowledge-*-smoke-test-cleanup.sh`), не общий
механизм. Три кандидата на интерфейс вызова, ни один не выбран: (a)
серверный скрипт (безопасно, соответствует уже принятому паттерну, но
только по SSH, неудобно с телефона); (b) chat-команда в Telegram/MAX
(удобно, но нужен способ сослаться на источник из чата — имя файла не
всегда уникально, sha256 не наберёшь на телефоне — и подтверждение
перед необратимым действием); (c) Panel UI (P8.5.8, сейчас `❌ не
реализовано "бессмысленно без данных"` — данные уже есть, это может
стать поводом сделать первую версию раньше остального Panel). Ждёт
решения владельца, прежде чем писать код — см. CLAUDE.md §1.
