...
## Текущая фаза: **Milestone A закрыт целиком.** Milestone B: Panel auth backend **закрыт и подтверждён живьём** (Telegram Login Widget → первый passkey → реальная сессия → дашборд). Дальше — MAX/n8n/Forgejo/GitHub CI.
