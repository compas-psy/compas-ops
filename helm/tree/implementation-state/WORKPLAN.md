...
**Осталось (только на сервере)**: включить `API_SERVER_KEY` у Hermes,
проверить живьём форму ответа `/v1/responses` (пока подтверждена только
по исходникам), доставить новый код Control Plane, живая приёмка §30.5
— всё по шагам в `scripts/hermes-enable-runbook.md`.
