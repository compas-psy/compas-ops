...
| §14.16 управление памятью словами | ⚠️ код готов, **ждёт живого деплоя**: «Забудь», «Верни в память», «Удали навсегда», «Исправь … : …» в обоих ботах владельца и в боте второго пользователя (`docs/KNOWLEDGE_ADMIN.md`) — команды над документами (переразбор, перенос, граф) приедут вместе с выдачей оригиналов §14.15 |
| §14.15 выдача оригинала документа | ⚠️ код готов, **ждёт живого деплоя**: поиск своих документов по имени и содержимому + выдача ИСХОДНЫХ БАЙТ со сверкой SHA256, свежий passkey на каждое скачивание, тенант из сессии. Заархивированное скачивается с пометкой «только для разбора». Выдача файла через бота не реализована — очередь исходящих умеет только текст |
| v3.8 P8.6.7 offboarding | ⚠️ код готов, **ждёт живого деплоя**: приостановка с отзывом сессий панели, возврат доступа, сброс passkey, выгрузка своего архива файлом, необратимое удаление (требует заранее приостановленного аккаунта и явного ответа про выгрузку), раскрытие сроков хранения в бэкапах со сверкой по `backup.sh` (`V3.8-DELTA.md`) — удаление сознательно НЕ идёт через реестр действий, разбор решения там же |
| ADR-019 L2 semantic atomizer (consolidation) | ⚠️ код готов, 596/596 тестов зелёных, **ждёт живого деплоя**: `knowledge/atomizer.py`, домено-агностичный (Ollama, `format: json`, fail-open), вызывается аддитивно из `ingest_text()`/`process_job()`; `health.knowledge_notes` заведена симметрично `health.knowledge_relations` (`scripts/setup-health-role.sh` не прогнан на живом сервере в этой версии) — backfill существующего корпуса и Knowledge Router в `probe()` не начаты (`docs/adr/ADR-019-knowledge-canonical-markdown-graphify.md`) |
...
**Пересмотрено 01.09.2026 (ADR-019):** L2 knowledge notes (consolidation)
раньше не создавались ни там, ни там — спека разрешала ingest'у
остановиться на `RAW + SOURCE + chunks + indexes` без LLM-вызова
("Lazy consolidation"). Это позволяло построить обычный chunk-based
поиск вместо семантического графа знаний — владелец указал на эту
архитектурную ошибку 01.09.2026. Теперь оба пути дополнительно вызывают
`atomizer.py::atomize_and_store()` — локальный (Ollama, 0 платных
AI-вызовов), домено-агностичный semantic atomizer создаёт L2 Markdown-
заметки с `[[wikilinks]]`, которые существующий `relations.py::
store_relations()` подхватывает без изменений. Подробности и
фазированный план — `docs/adr/ADR-019-knowledge-canonical-markdown-
graphify.md`.
