...
| P8.5.7 Telegram/MAX ingress вложений (spool → RAW → register_file_for_ingest, двухшаговый диалог домена) | MAX-сторона ✅ задеплоена, подтверждена живьём (реальный PDF, меню доменов, корректная форма вебхука с первого раза, `ADR-021`). Telegram-сторона ⚠️ код готов (два новых internal-эндпоинта + `helm-control`), untestable локально, ждёт живого деплоя и первой проверки — заменит текущее agentic-чтение файлов чифом, не будет сосуществовать с ним (решение владельца) |
