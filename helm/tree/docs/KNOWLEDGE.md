...
| v3.8 P8.6.5 Panel roles | 🟡 частично: owner-раздел «Система → Пользователи» (список/инвайт/suspend/квоты, scope-привязанный passkey step-up) готов, **ждёт живого деплоя**; Knowledge-only оболочка для KNOWLEDGE_USER не реализована — у secondary-пользователя нет способа войти в панель, выбор пути enrollment'а за владельцем (`V3.8-DELTA.md`) |
