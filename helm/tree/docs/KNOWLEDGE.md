...
| P8.5.7 Telegram/MAX ingress вложений (spool → RAW → register_file_for_ingest, двухшаговый диалог домена) | MAX-сторона ✅ задеплоена, подтверждена живьём (реальный PDF, меню доменов, корректная форма вебхука с первого раза, `ADR-021`). Telegram-сторона ❌ не реализована — ждёт живой разведки хука Hermes (`scripts/knowledge-telegram-attachment-recon.sh`); заменит текущее agentic-чтение файлов чифом, не будет сосуществовать с ним (решение владельца) |
