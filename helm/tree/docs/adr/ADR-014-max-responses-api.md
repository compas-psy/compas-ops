# ADR-014. MAX-канал: Control Plane как клиент встроенного API Hermes
...
`ADR-014-max-bridge.md`) предлагала строить новый Hermes-side плагин
...
- 8642 (порт API-сервера Hermes) СЛУШАЕТ `0.0.0.0`, не `127.0.0.1` —
  найдено живьём 29.08.2026: Hermes работает на хосте, Control Plane в
  Docker-контейнере со своим network namespace, и `127.0.0.1` внутри
  контейнера означает сам контейнер, а не хост (тот же класс проблемы,
  что уже дважды всплывал в этой сессии — helm-core на P2, n8n — только
  в обратную сторону: контейнер стучится к хосту, а не наоборот).
  Защита от интернета — не bind-адрес, а хостовый `ufw`
  (`default deny incoming`, разрешены только 22/80/443 + точечное
  правило на 8642 из подсети докер-моста `172.18.0.0/16`, никак не из
  0.0.0.0/0). Явно входит в `NEVER_PUBLIC_PORTS` (`test_perimeter.py`)
  на случай, если Caddy/compose когда-нибудь попытаются его
  проксировать — но реальная защита сейчас на уровне хоста, вне
  видимости этого репозитория.
- Контейнер `helm-core` резолвит хост через `host.docker.internal`
  (`extra_hosts: host-gateway` в docker-compose.yml), а не по IP моста
  напрямую — IP может смениться при пересоздании сети, имя нет.
