...
**Честно ещё не проверено**: сам `TelegramSender` на реальном сервере
(нужен повторный деплой `helm-core` + повторная отправка файла), и
критическая проверка, которую владелец явно попросил, — что чиф на
живом Telegram-сообщении с вложением НЕ вызывается напрямую.
