# ADR-024. ZIP batch ingest: безопасная распаковка + durable очередь + exactly-once финал
...
MAX). Номер `ADR-024` — тот же, что называет сама спека v3.7 в
`## ADR-024 ZIP batch ingest`, переиспользование не требуется.
