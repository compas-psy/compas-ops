# HELM Knowledge — Ingest

Как материал попадает в базу знаний. Документ о том, что РЕАЛЬНО
реализовано, и о том, что явно отложено, — не пересказ спеки как факта.
Даты живых подтверждений указаны по месту.

Область: **L0 → L1** (оригинал → читаемый Markdown → чанки → индексы).
Оба пути ingest'а в конце дополнительно вызывают
`atomizer.py::atomize_and_store()` — экспериментальный semantic-v1,
который с принятием HELM v4.0 RESCUE признан ненормативным и в живой
базе не создал ни одной заметки. Состояние семантического слоя —
`docs/KNOWLEDGE.md` и `implementation-state/V4.0-RESCUE-DELTA.md`;
целевой контракт — `docs/spec/CURRENT.md`, §14.4–§14.12.
...
**Повторная отправка того же файла (вопрос владельца 30.08.2026).**
`register_file_for_ingest()` и так не создавал вторую ingest job для
уже известного SHA256 — но `resolve_pending_domain()` проверял это
только ПОСЛЕ переноса файла из spool в `raw/<domain>/`, что давало два
скрытых дефекта: (1) владелец получал «Разбор запущен» на файл, для
которого job не создавался вовсе; (2) если тот же файл отправить
повторно в ДРУГОЙ домен, копия физически ложилась в `raw/<новый
домен>/` (имя файла на диске — sha256, без домена в пути роли не
играет), а `KnowledgeSource` в БД оставался привязан к первому домену —
файл во втором домене превращался в сироту, которую ничто не чистит.
Исправлено: проверка по `pending.sha256` теперь идёт ДО переноса файла
— найден существующий source → spool-копия удаляется, pending
снимается, ответ владельцу — новый статус `duplicate` («Этот файл уже
есть в базе (домен «X») — повторно не сохраняю и не разбираю»), никакой
файл никуда не переносится. Дедуп по-прежнему глобальный, не per-domain
(так и было задумано `register_file_for_ingest()`) — переклассификация
существующего source в другой домен этой веткой не делается, это
отдельная, ещё не реализованная операция (см. «Что дальше» ниже).
