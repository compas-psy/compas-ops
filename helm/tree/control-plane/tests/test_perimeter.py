...
#: гейт Control Plane (регистрацию, дедуп, owner-проверку). Hermes
#: работает на ХОСТЕ, не в Docker (§4.6 этого набора проверок его не
#: видит вовсе — здесь он только затем, чтобы Caddy/compose никогда не
#: попытались проксировать его как обычный сервис). Реальная защита
#: 8642 от интернета — ufw на хосте (default deny incoming, разрешены
#: только 22/80/443; отдельное правило пускает 8642 ТОЛЬКО из подсети
#: докер-моста, найдено и настроено 29.08.2026 при первом живом вызове
#: /v1/responses из контейнера helm-core) — это состояние хоста, не
#: репозитория, и ни один тест этого файла его не проверяет.
