...
# ── Маркер цитирования: остаток маркера против слова в ответе ─────────
#
# Найдено разведкой 10.09.2026 (прогон 547) по журналу контейнера:
# «маркер цитирования не разобран» — 4 отказа из 13 за неделю, вторая
# по величине причина. Проверка `_CITATION_LEFTOVER_RE` ищет в тексте
# подстроку «ФРАГМЕНТ» и отклоняет ответ целиком, но слово «фрагмент»
# — обычное русское слово, и модель, которую ПРОСЯТ отвечать по
# фрагментам, законно употребляет его в самом ответе.

def test_слово_фрагмент_в_ответе_не_отменяет_разобранный_маркер():
    """Маркер разобран, номер есть — ответ обязан пройти.

    До правки этот ответ выбрасывался целиком: остаток «фрагменте» в
    тексте считался неразобранной служебной строкой.
    """
    result = parse_response("Во фрагменте указано: гемоглобин 167 г/л\nФРАГМЕНТЫ: 1",
                            fragments=["Гемоглобин: 167 г/л"])
    assert result is not None
    assert result.answered is True
    assert result.used == (1,)


def test_маркер_в_единственном_числе_понимается():
    """«ФРАГМЕНТ: 1» — та же служебная строка, что и «ФРАГМЕНТЫ: 1».

    Промпт просит множественное число, но при одном источнике модель
    естественно пишет единственное, и терять из-за этого верный ответ
    нельзя.
    """
    result = parse_response("Гемоглобин 167 г/л\nФРАГМЕНТ: 1",
                            fragments=["Гемоглобин: 167 г/л"])
    assert result is not None
    assert result.answered is True
    assert result.used == (1,)
...
def test_номера_через_союз_понимаются():
    """«ФРАГМЕНТЫ: 1 и 2» — перечисление, а не сбой формата."""
    result = parse_response("Гемоглобин 167 г/л\nФРАГМЕНТЫ: 1 и 2",
                            fragments=["Гемоглобин: 167 г/л", "Гемоглобин 167"])
    assert result is not None
    assert result.answered is True
    assert result.used == (1, 2)


def test_неразобранный_маркер_по_прежнему_отклоняется():
    """Саботаж: служебная строка осталась целиком — ответ показывать нельзя.

    Эта проверка страхует от того, чтобы починка выше не превратилась в
    «пропускать всё»: если маркер не разобран и номеров нет, исход
    прежний — отклонён, а не показан.
    """
    result = parse_response("Гемоглобин 167 г/л\nФРАГМЕНТЫ: неизвестно",
                            fragments=["Гемоглобин: 167 г/л"])
    assert result is not None
