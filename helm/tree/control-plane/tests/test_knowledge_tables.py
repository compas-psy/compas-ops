...
# --- Вторая форма бланка, измеренная прогоном 491 -----------------------
#
# Тот же лабораторный бланк другой вёрстки: пустая строка между
# колонками, колонка отклонения и текстовая колонка нормы, порядок
# колонок не тот. Прежние правила обрывались на первой же строке, и
# значение 8.4 оставалось числом без имени — из-за чего живой ответ
# владельцу 09.09.2026 был отклонён как «значение при чужом объекте».

COLUMN_BLOCK = ("Холестерин общий\n"
                "см. комментарий\n"
                "\n"
                "ммоль/л\n"
                "\n"
...
def test_columns_separated_by_blank_lines_still_bind():
    assert restore_table_rows(COLUMN_BLOCK) \
        == "Холестерин общий: ↑ 8.4 ммоль/л (см. комментарий)"


def test_the_deviation_flag_survives_into_the_row():
    """«↑» — не украшение: это признак выхода за норму."""
    assert "↑" in restore_table_rows(COLUMN_BLOCK)


def test_a_paragraph_break_ends_the_row():
    """Одна пустая строка — разделитель колонок, две — конец таблицы."""
    text = "Заголовок\n\n\n5.4\nммоль/л"
    assert restore_table_rows(text) == text


def test_a_wrapped_label_is_joined_by_its_open_bracket():
    text = ("Холестерин-ЛПНП (липопротеины\n"
            "низкой плотности)\n"
            "\n"
            "↑\n"
            "\n"
...
def test_the_next_rows_label_is_not_eaten_as_a_reference_note():
    """Главный риск правила о текстовой норме — съесть соседнюю строку.

    «Гемоглобин» тоже короткий и без чисел, и следом за ним тоже идёт
    значение. От нормы его отличает прописная буква: названия
    показателей в бланке пишутся с прописной, пометки — со строчной.
    """
    text = "Холестерин общий\nГемоглобин\n164\nг/л"
    assert restore_table_rows(text) == "Холестерин общий\nГемоглобин: 164 г/л"


def test_a_note_without_a_cell_after_it_is_not_a_note():
    """Короткая строчная строка посреди прозы нормой не становится."""
    text = "Заголовок\nвсё это было давно\nи уже неправда"
    assert restore_table_rows(text) == text


def test_prose_between_label_and_value_leaves_the_row_alone():
    """НАЗВАННОЕ ОГРАНИЧЕНИЕ, А НЕ НЕДОСМОТР.

    В том же бланке между названием ЛПВП и его значением стоит абзац
    пояснения. Перешагнуть через него — значит разрешить склейку через
    прозу, ту самую, против которой стоит проверка длины. Такая строка
    остаётся неcклеенной, и ответа по ней не будет — это честнее, чем
    значение, притянутое через абзац текста.
    """
    text = ("Холестерин-ЛПВП\n"
            "Пороговые значения для мужчин: более 1 ммоль/л (40 мг/дл);\n"
            "\n"
            "1.77\n"
            "\n"
            "ммоль/л")
