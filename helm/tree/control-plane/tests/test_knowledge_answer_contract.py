...
def test_unrecognised_question_still_goes_the_old_way(monkeypatch):
    ...
    # Исход стал LOCAL_NOT_FOUND 07.09.2026: вопрос про АНАЛИЗ — вопрос
    # к записям, и пустой поиск по нему не даёт права заплатить (то же
    # правило, что закрыло «Уровень холестерина по липидному профилю?»).
    # Проверяемое здесь свойство — что вопрос дошёл до поиска по чанкам,
    # — держится строкой выше и не изменилось.
