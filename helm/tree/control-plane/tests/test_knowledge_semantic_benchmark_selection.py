...
from helm_core.knowledge.semantic_benchmark_selection import (
    CandidateResult, ResourceStats, evaluate_hard_gates, select_winner,
)
...
def _candidate(model: str, *, peak_rss_mb: float = 1000.0, oom=False, degraded=False,
              litellm_calls=0, openrouter_calls=0, **report_kwargs) -> CandidateResult:
    report = _report(**report_kwargs)
    report.model = model
    return CandidateResult(
        model=model, quant_tag="q4_0", golden=report,
        resources=ResourceStats(peak_rss_mb=peak_rss_mb, oom_occurred=oom,
                                other_services_degraded=degraded),
