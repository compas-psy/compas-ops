...
def test_prose_with_a_measurement_is_not_a_table_row():
    """ИЗМЕРЕНО ПРОГОНОМ 500, живой корпус.

    В выписку про холестерин попало предложение из заключения
    кардиолога. Измерение в нём есть — но это давление, а слово
    «холестерина» стоит в конце фразы, а не в названии показателя.
    Название — до первого двоеточия, и слово вопроса обязано стоять
    именно там.
    """
    prose = ("препараты  на  постоянной  основе  не  принимает.  "
             "Адаптирован  к  120/80  мм  рт.ст.  Уровень  холестерина")
    done = run_measurements("какой у меня был холестерин?", [prose, LIPID_ROWS])
    assert done is not None
    assert "Адаптирован" not in done.text
    assert "8.4 ммоль/л" in done.text
