...
def request_rederivation(session: Session, *, source_id: uuid.UUID,
    ...
    ОСНОВНОЙ ВЫЗЫВАЮЩИЙ — `reparse.py`, и вот почему без этой функции
    он не работает. Ключ уникальности задания — «пользователь, источник,
    содержимое, версия», где содержимое это sha256 ФАЙЛА. После
    исправления парсера байты те же и версия та же, поэтому
    `enqueue_semantic` считает работу выполненной и нового задания не
    создаёт — хотя разобранный текст стал другим. Работа обязана стать
    невыполненной у СУЩЕСТВУЮЩЕГО задания, и делает это здесь.

    Прежде здесь было написано, что штатный переразбор — подъём
    `SEMANTIC_VERSION`, а это исключение. Прогон 469 показал обратное:
    подъём версии до разбора файла не доходит вовсе (см. докстринг
    `derivation.py`).

    Незаконченная ревизия закрывается ЯВНО — в отличие от прежнего
    автоматического закрытия при возврате брошенного задания, которое
    молча уничтожало прогресс порций, — и разбор начинается с первого
    окна.
    """
