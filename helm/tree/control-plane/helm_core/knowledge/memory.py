...
#:
#: Триггер ищется в начале ЛЮБОГО предложения, а не только всего текста.
#: Распоряжение владельца 07.09.2026: «для меня, как для пользователя,
#: не имеет значения, какое слово идёт первым». Живой случай — надиктованное
#: «Это тестовое сообщение. Запомни, что меня зовут Илья Мартынов»:
#: команда не опознавалась, потому что перед ней была фраза-разгон,
#: которую человек в живой речи произносит не задумываясь.
#:
#: Но и не «где угодно в тексте»: `(?<=[.!?\n])` оставляет вопрос
#: вопросом. «А ты запомни, что я говорил?» — здесь «запомни» внутри
#: предложения, это не команда сохранить, и превращать такой вопрос в
#: запись нельзя.
_REMEMBER_PREFIX = re.compile(
    ...
    r"|не\s+забудь(?:те)?\b"
    r")\s*[:,\-—]?\s*"
    # «Запомни, ЧТО меня зовут Илья» — служебное «что» принадлежит
    # команде, а не факту. Без него живая голосовая заметка 07.09.2026
    # легла в память дословно как «что я живу на улице Ландышева, и
    # меня зовут Илья», и ответом на «Как меня зовут?» стала эта же
    # фраза с оборванного места. `\s+` после, а не `\b`: иначе
    # «Запомни что-то» превратилось бы в «-то».
    r"(?:что(?:бы)?\s+)?",
    re.IGNORECASE,
)
...
def detect_remember_command(text: str) -> str | None:
    ...
    Ищется первое вхождение триггера в начале предложения, и payload —
    всё, что после него: разгонная фраза перед командой («Это тестовое
    сообщение. Запомни, что …») отбрасывается вместе с ней, потому что
    фактом является сказанное ПОСЛЕ.
    ...
    match = _REMEMBER_PREFIX.search(text)
    if match is None:
        return None
