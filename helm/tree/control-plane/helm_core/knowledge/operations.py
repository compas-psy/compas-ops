...
#: Длина названия показателя в строке таблицы. Восьмидесяти хватает на
#: «07.10.2023  Липидный профиль (ммоль/л) Холестерин общий» — дату,
#: раздел и название вместе; связная фраза до двоеточия длиннее.
MAX_ROW_LABEL_CHARS = 80
...
def run_measurements(question: str, texts: list[str]) -> OperationAnswer | None:
    ...
    ГРАНИЦЫ. Берётся именно СТРОКА ТАБЛИЦЫ: «название: значение
    единица». Название — до первого двоеточия, и слово вопроса обязано
    стоять ИМЕННО В НЁМ.

    ПОЧЕМУ НЕ «ГДЕ-НИБУДЬ В СТРОКЕ» (прогон 500). Первая редакция брала
    строку, если та несла измерение и слово вопроса где угодно. В
    выписку про холестерин попало предложение из заключения кардиолога:
    «…Адаптирован к 120/80 мм рт.ст. Уровень холестерина» — измерение в
    нём есть (давление), слово есть, строкой таблицы оно не является, и
    к вопросу о значении холестерина отношения не имеет.

    Одна строка — `None`: с одной справляется синтез, и подменять
    связный ответ выпиской незачем. Дубли по строке убираются, порядок
    сохраняется: он идёт от свежести источника, а не от нашего мнения.
    """
    wanted = {stem for stem in _stems(question)
              if len(stem) >= 4 and stem not in _FUNCTION_STEMS}
    if not wanted:
    ...
    for index, text in enumerate(texts, start=1):
        ...
        for line in text.splitlines():
            ...
            if not separator or len(label) > MAX_ROW_LABEL_CHARS:
                continue
            if not MEASUREMENT_RE.search(value):
                continue
            if not (wanted & _stems(label)):
                continue
            seen.add(row)
            found.append(row)
            contributed = True
