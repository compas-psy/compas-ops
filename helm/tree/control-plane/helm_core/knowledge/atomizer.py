...
_SYSTEM_PROMPT = (
    ...
    "Верни СТРОГО JSON-массив, без пояснений до или после, без markdown-"
    "разметки вокруг:\n"
    '[{"slug": "короткое имя, 2-4 слова", '
    '"type": "ENTITY|PERSON|ORGANIZATION|PLACE|EVENT|CONCEPT|FACT|DECISION", '
    '"text": "1-3 предложения, что именно сказано в тексте про это", '
    '"links": ["slug другой сущности ИЗ ЭТОГО ЖЕ ответа, с которой есть явная связь"]}]\n\n'
    "Если в тексте не за что зацепиться — верни []."
)
...
def _parse_atoms(raw_json: str) -> list[AtomizedAtom]:
    ...
    if not isinstance(data, list):
        raise AtomizerUnavailable("ответ модели — не JSON-массив")
