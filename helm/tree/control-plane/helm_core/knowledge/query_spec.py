...
class QuerySpec:
    ...
    #: ЛИБО его назвал сам вопрос («по книге Линде…»). Пусто — искать
    #: везде.
    focus_source_ids: tuple[str, ...] = field(default_factory=tuple)
    #: Название источника, прозвучавшее В САМОМ ВОПРОСЕ, ещё не
    #: разрешённое в идентификаторы. Разрешает его исполнитель: здесь
    #: базы нет, а сопоставлять название с корпусом можно только по
    #: корпусу. Пусто — вопрос источника не называл.
    source_hint: str = ""
...
#: Слова-носители документа. Закрытый класс ФОРМЫ, а не словарь
#: предмета: «книга», «выписка», «анализ» говорят, что дальше идёт имя
#: источника, и ничего не говорят о теме. Та же граница, по которой
#: заведены «сколько» и «все» в `operations.py`.
_SOURCE_KIND = (r"книг\w*|документ\w*|файл\w*|выписк\w*|анализ\w*|"
                r"заметк\w*|записк\w*|стать\w*|письм\w*|отчёт\w*|отчет\w*|"
                r"протокол\w*|заключени\w*|консультаци\w*")

#: Имя источника после слова-носителя: либо в кавычках, либо подряд
#: идущие слова с заглавной буквы. Заглавная — единственный признак
#: имени, который есть в самом тексте вопроса; строчное слово имя
#: обрывает, иначе «по книге Линде что такое ЭОТ» утащило бы в название
#: весь вопрос.
#: Регистр НЕ игнорируется целиком: заглавная буква — единственный
#: признак имени в тексте вопроса, и `re.IGNORECASE` на весь шаблон
#: убивал бы правило. Нечувствительна только приставка «по книге».
_SOURCE_REF_RE = re.compile(
    r"(?i:\b(?:по|в|во|из|среди)\s+(?:" + _SOURCE_KIND + r")\s+)"
    r"(?:[«\"']([^»\"']{2,80})[»\"']|((?:[А-ЯЁA-Z][^\s,.;:?!]*\s*){1,4}))")


def detect_source_hint(question: str) -> str:
    """Название источника, которым вопрос ограничивает поиск.

    Распоряжение владельца 07.09.2026: «по книге Линде что такое ЭОТ?»
    получило в ответ «Глава 8. Зависть» — историю про детскую книжку о
    принцессе. Ограничение источника система понимает (`focus_source_
    ids`), но заполняет его только из контекста разговора; сам вопрос
    условием не считался и просто пропадал.

    Чего здесь НЕТ и не будет: словаря названий книг и авторов.
    Распознаётся форма — «предлог + слово-носитель + имя», — а
    сопоставление имени с корпусом делает исполнитель по самому корпусу.
    """
    match = _SOURCE_REF_RE.search(question)
    if match is None:
        return ""
