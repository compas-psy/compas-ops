...
#: Дерево Vault пишут ДВА процесса под разными UID: `helm-core` (10000)
#: и `helm-knowledge-worker` (10002). Общего у них только группа `helm`
#: (GID 1001, `group_add` в docker-compose.yml), и `knowledge-bootstrap.
#: sh` ровно на это и рассчитан: `chmod -R 770` плюс setgid на каталогах.
#:
#: Но bootstrap отрабатывает ОДИН раз, а каталоги создаются в рантайме
#: (`users/<tenant>/memory/` появляется на первом «Запомни»). У обоих
#: контейнеров umask 0022 — значит `mkdir` в рантайме даёт 755+setgid:
#: группа наследуется, право записи для неё теряется. Каталог, созданный
#: одним контейнером, второму доступен только на чтение.
#:
#: НАЙДЕНО живым сбоем 07.09.2026 (разведка, прогон 406): голосовое
#: «Запомни …» падало в воркере на `[Errno 13] Permission denied:
#: /opt/helm-knowledge/users/<tenant>/memory/<id>.md` — каталог создал
#: helm-core на текстовом «Запомни», воркер писать в него не мог.
#: Владелец при этом получал «не получилось расшифровать», хотя
#: расшифровка прошла.
#:
#: 0o007 — ровно то же правило, что уже записано в bootstrap: каталог
#: 770, файл 660, «свой» и «группа» пишут, чужие не видят.
VAULT_UMASK = 0o007


def share_new_files_with_group() -> None:
    """Вызывается один раз при старте каждого процесса, который пишет в
    Vault. Не в `write_file()`: umask — свойство процесса, а не отдельной
    записи, и менять его на каждый файл значило бы гонку между потоками
    FastAPI."""
    os.umask(VAULT_UMASK)
