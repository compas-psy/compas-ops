...
def format_unverified(filenames: list[str]) -> str:
    ...
    """
    cites = _cite_list(filenames)
    # Формулировка НЕ про значения. Прогон 445: на «дай примеры того, как
    # бороться с неуверенностью» пришло «значение не удалось однозначно
    # связать» — а никакого значения в вопросе и не было. Отклонить ответ
    # можно по числу, по названию и по отсутствию опоры на фрагмент, и
    # текст обязан покрывать все три, а не только первый.
    text = ("Нашёл записи по вашему вопросу, но подтвердить ответ по ним не смог: "
            "то, что получилось, их текстом не подтверждается.")
    if cites:
        text += f"\n\nСмотрел: {cites}"
    return text


def format_missing_comparison_side(side: str, filenames: list[str]) -> str:
    """Сравнивать не с чем: одной из названных сторон в записях нет.

    Отдельный текст, а не «ответа не нашёл»: владелец спросил про две
    вещи, и ему нужно знать, КОТОРОЙ не хватает. «Ничего не нашёл»
    отправило бы его переспрашивать всё сравнение целиком, хотя одна
    сторона в памяти есть.
    """
    cites = _cite_list(filenames)
    text = (f"Сравнить не могу: про «{side}» в ваших записях ничего нет.\n\n"
            f"Спросите про вторую сторону отдельно — по ней записи есть.")
    if cites:
        text += f"\n\nСмотрел: {cites}"
    return text
