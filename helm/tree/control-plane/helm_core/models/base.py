...
class KnowledgeAnswerMode(enum.StrEnum):
    ...
    `S1` спекой v4.0 не задан — добавлен 05.09.2026 вместе с
    детерминированным структурным ответом (D-3 в SPEC_DEVIATION.md).
    Отдельное значение, а не Z0/Z1, потому что метрика §14.14 обязана
    отличать «ответили доказанным фактом из графа» от «показали
    найденный кусок текста»: без этого различия нельзя увидеть, растёт
    ли доля структурных ответов вообще.
    """
    ...
    Z2 = "Z2"
    S1 = "S1"
    #: Вопрос о данных владельца, ответа в памяти нет. Добавлен
    #: 06.09.2026: «Отсутствие находок не является моим разрешением
    #: оплатить ответ». Отдельное значение, а не отсутствие строки:
    #: метрика §14.14 обязана видеть, СКОЛЬКО раз память честно
    #: промолчала, иначе рост доли бесплатных ответов не отличить от
    #: роста числа вопросов, оставшихся без ответа вообще.
    N0 = "N0"
    #: Вопрос не имеет ответа сам по себе и требует уточнения («что ТАМ
    #: прописал врач» — в каком «там»?). Отдельно от N0: там ответа нет
    #: в данных, здесь его нет в вопросе, и лечится это разными вещами.
    Q0 = "Q0"
    C1 = "C1"


class KnowledgeChatModeValue(enum.StrEnum):
    """Режим чата: из чего разрешено отвечать.

    `MEMORY` — только собственные записи владельца, платная модель не
    вызывается. `PAID` — платный переход разрешён явно.

    Умолчание — `MEMORY`, и это не настройка, а политика (распоряжение
    владельца 07.09.2026): «Отсутствие контекста не является
    разрешением». Прежний плагин выводил право платить из
    ВНУТРИПРОЦЕССНОГО словаря последних ходов: перезапуск шлюза стирал
    память о разговоре, и потерянный контекст снова открывал оплату.
    Режим лежит в базе именно поэтому — он обязан пережить перезапуск.
    """
...
class EntityIdentityMatch(enum.StrEnum):
    """Чем доказано, что узел — та же личность (R6, §14.7).

    Оба значения — сильное тождество, и других здесь быть не должно:
    §14.7 разрешает автослияние ТОЛЬКО при совпадении сильной личности.
    Похожесть, общая фамилия и вектор — не доказательство, они уходят в
    `EntityResolutionReason` кандидатом на решение человека.
    """

    #: Дословно совпала нормализованная подпись при том же `entity_type`.
    NORMALIZED_LABEL = "normalized_label"
    #: Подпись дословно совпала с ПОДТВЕРЖДЁННЫМ алиасом личности
    #: (`knowledge_entity_aliases`), а не с догадкой о написании.
    ALIAS = "alias"


class EntityResolutionReason(enum.StrEnum):
    """Почему пара похожа, но слита НЕ будет (R6, §14.7).

    Кандидат — это не «почти слияние», а зафиксированный вопрос к
    человеку. Каждое значение здесь — случай, который §14.7 называет
    запрещённым для автоматики поимённо.
    """

    #: «Иванов» и «Иванов Пётр Сергеевич»: одна фамилия — не одно лицо.
...
class EntityResolutionStatus(enum.StrEnum):
    """Судьба кандидата. Решает человек, не агент (устав §6)."""
