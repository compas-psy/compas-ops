...
class ExecutionContext(Protocol):
    ...
    `session` — та же сессия БД, в которой ведётся учёт одобрения. Она
    здесь ради действий, меняющих данные: удаление аккаунта обязано
    попасть в ту же транзакцию, что и перевод одобрения в EXECUTED.
    Открой исполнитель свою сессию — данные удалились бы, а запись об
    исполнении могла бы не сохраниться, и необратимое действие осталось
    бы без следа. Действиям, которым БД не нужна, поле просто не нужно.
    """
