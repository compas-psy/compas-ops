...
# СТРОЖЕ, чем _ABSOLUTE_DATE_RE в semantic_extract.py:319, и намеренно.
# Прогон 306 показал: «\d{1,2}[./]\d{1,2}» ловит «5.4 ммоль/л» — в
# медицинских текстах это лабораторное значение, а не дата, и потолок
# блока 3 был завышен. Здесь дата — это четырёхзначный год (19xx/20xx),
# ДД.ММ.ГГГГ целиком либо русское название месяца. Тот же дефект живёт и
# в продакшн-гейте П3: он принимает «5.4» за подтверждение даты — это
# находка по гейту, записана в §6, здесь не правится.
ABS='\m(19|20)\d{2}\M|\d{1,2}[./]\d{1,2}[./](19|20)\d{2}|январ|феврал|март|апрел|\mма[ей]|июн|июл|август|сентябр|октябр|ноябр|декабр'
...
psql "select left(regexp_replace(text, '\s+', ' ', 'g'), 200)
      from health.knowledge_chunks where text ~* '${ABS}' and length(text) >= 80
      order by length(text) limit 5"
