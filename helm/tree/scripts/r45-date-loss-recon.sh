...
#
# Что этот замер различает, а что нет — честно:
#   П4 отделяется точно (блок 1: date_precision='unknown' при пустом
#       occurred_at_start — это разобранная неудача разбора);
#   П2+П3 вместе ограничиваются сверху (блок 2: rejected_count считает
#       ВСЕ причины отказа, не только датные — причины не сохраняются,
#       только их число, semantic_publish.py:374);
#   П1 следа не оставляет вовсе и получается вычитанием.
# Разделить П2 и П3 между собой можно только повторным прогоном
# извлечения с сохранением причин — это следующий шаг, и он нужен лишь
# если блок 2 покажет ненулевые отказы.
#
...
echo "############ 1. ТОЧНОСТЬ ДАТЫ × НАЛИЧИЕ ДАТЫ (П4) ############"
