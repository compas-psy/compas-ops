...
try:
    ...
    ok = "ключом доступа" in text or "Какой из них?" in text
    record("3", "оригинал выдаётся с ключом доступа", ok,
           "ответ ведёт к оригиналу под ключом" if ok else "ответ не про файл")
