#!/usr/bin/env python3
"""R4.7 (владелец 03.09.2026) — «Frozen relation benchmark v3 НЕ удалять.
Пересчитать его для deterministic compiler. Основной критерий: relation
precision >= .90. Recall показать диагностически. Не расширять fixture
corpus ради улучшения результата.»

Изолирует compiler от pass1: entities/atoms берутся ИЗ fixtures как
есть (`RelEntity`/`RelAtom` — то, что уже было бы «идеально извлечено»),
не из живого прогона LLM — offline, без сервера, без NLI.

Методология оценки (не путать с NLI-версией того же benchmark,
`r4-f1-2-nli-benchmark-v3.sh` — там hypothesis сравнивались с моделью
через entailment score; здесь compiler либо производит ребро, либо нет):
  TP = произведённое ребро совпадает с объявленным `entailed` (from,
       relation_type, to) — точно, не по одному полю.
  FP = произведённое ребро НЕ совпадает ни с одним `entailed` —
       независимо от того, есть ли оно в `not_entailed` явно (сам
       факт «не entailed» достаточен, `not_entailed` — только
       читаемая причина для отчёта, не единственный источник «плохо»,
       иначе непроверенные-но-неверные рёбра остались бы бесплатными).
...
def main() -> int:
    ...
    print()
    print(f"TOTAL entailed (gold): {total_gold}")
