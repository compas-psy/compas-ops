...
echo
echo "############ МИГРАЦИЯ ############"
# cmpas.ru ВКЛЮЧЁН РЕШЕНИЕМ ВЛАДЕЛЬЦА 09.09.2026 («практику тоже
# переноси — это моё решение»). Прежде он был исключён: в его истории
# лежит утёкший приватный ключ (H-260818-02), и перенос копирует эту
# историю ещё в одно место. Решение принято, оговорка остаётся фактом:
# отзыв ключа переносом не отменяется и не заменяется.
#
# Уже перенесённые репозитории в списке остаются намеренно: миграция
# отвечает «уже существует» и заново сверяет refs с GitHub — это дешёвая
# перепроверка целостности, а не повторная работа.
sudo python3 /opt/helm/scripts/forgejo-migrate.py compas-voice zapiski signalAI-mobileApp cmpas.ru
