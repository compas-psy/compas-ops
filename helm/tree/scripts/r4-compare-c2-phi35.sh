#!/bin/bash
# HELM v4.0 RESCUE · R4.6.D candidate 2/3 — phi3.5 (Microsoft, тренирован
# преимущественно на синтетических данных — иной подход к data curation,
# чем у Qwen/Alibaba, Gemma/Google, Llama/Meta.
#
...
# 3B — заметно меньше qwen2.5:7b, тот же 6g потолок, что использовался
# для промежуточных кандидатов run 210 (gemma2:2b/qwen2.5:3b), с
# запасом сверх голых весов на KV-cache/overhead.
echo "=== временно поднимаем лимит ollama до 6g ==="
sudo docker update --memory=6g --memory-swap=6g "$(OLLAMA_CID)"
echo "=== ollama pull phi3.5 ==="
sudo docker compose exec -T ollama ollama pull phi3.5
