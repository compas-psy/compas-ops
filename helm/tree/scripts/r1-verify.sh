...
#
# ИСПРАВЛЕНО 02.09.2026: раньше список читался через `< <(psql ...)`.
# Внутри подстановки процессов код возврата основной оболочке не виден —
# упавший запрос выглядел бы как пустая выборка, и проверка напечатала бы
# «целых 0» вместо «запрос не выполнился». Тем же способом ложный PASS
# получился в тесте восстановления (прогон #152). Пишем в файл и
# проверяем статус.
list=$(mktemp)
if ! psql -tAc "select sha256, raw_path from knowledge_sources where domain = 'health'" > "$list"; then
  echo "  ЗАПРОС НЕ ВЫПОЛНИЛСЯ — проверка файлов не проводилась"
  rm -f "$list"
  exit 1
fi
