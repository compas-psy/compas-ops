...
#
# Отказ приходит не на весь запрос, а на отдельные блоки (три разных
# хэша, EOF и 500 вперемешку) и только когда их много. Похоже на
# ограничение частоты запросов на стороне Яндекса под всплеск, а не на
# нехватку таймаута: таймауты уже подняты и не помогли.
#
# Отсюда две настройки, обе про темп, а не про терпение: не больше
# четырёх запросов в секунду и две параллельные передачи вместо пяти по
# умолчанию (restic задаёт их бэкенду rclone через -o rclone.connections
# ниже). Гипотеза проверяется одним живым прогоном; если 500 останутся —
# дело не в темпе, и это уже вопрос смены хранилища (H-260902-01).
export RCLONE_TPSLIMIT=4
