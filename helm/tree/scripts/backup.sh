...
#
# Отсюда: поднять порог, ниже которого rclone буферизует тело целиком и
# отправляет обычным PUT с Content-Length. По умолчанию он 100 КиБ, то
# есть почти всё идёт потоком. Пачки restic — около 16 МиБ, 64 МиБ с
# запасом покрывают их; при двух параллельных передачах это 32 МиБ
# памяти на сервере, где свободно почти девять гигабайт.
