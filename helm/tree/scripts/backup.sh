...
# НАЙДЕНО 02.09.2026 (прогон 150). Сам бэкап прошёл — `snapshot 10a85e55
# saved`, — а скрипт всё равно упал, и упал здесь: репозиторий держал
# эксклюзивный лок от 01.09 14:37, оставшийся от оборванного прогона.
# `restic backup` берёт нестрогий лок и на чужой не смотрит, а
# `forget --prune` требует эксклюзивный и ждать не стал.
#
# Последствие было хуже самого отказа: отметка `last-backup` ставится
# ниже, и из-за падения не ставилась. То есть успешный бэкап выглядел
# как несостоявшийся — ровно та ложь в отчётности, из-за которой сегодня
# уже пришлось переписывать выводы.
#
# `restic unlock` без ключей снимает только те локи, чей процесс мёртв.
# Живой параллельный прогон он не тронет.
restic unlock
