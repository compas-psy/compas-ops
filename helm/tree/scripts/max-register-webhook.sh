...
# Ответ печатается без секрета: сам секрет MAX обратно не возвращает, но
# тело ответа сюда попадает как есть, поэтому проверяем глазами.
echo "HTTP $HTTP"
cat /tmp/max-subscribe.out
echo
...
# действительно записана на стороне MAX.
curl -sS "$API/subscriptions" -H "Authorization: $TOKEN"
echo
echo
