...
#: `< /dev/null` не декорация. НАЙДЕНО 02.09.2026 (прогон #155): после
#: добавления `-i` docker exec стал читать stdin, а внутри цикла stdin —
#: это сам список файлов. Первый же запрос съел его целиком, цикл
#: отработал одну итерацию из девяноста и напечатал «оригиналов 1 из 90».
#: Ноль пропаж при одной проверке — снова тихий отказ, снова пойманный
#: собственным утверждением. Команда внутри цикла обязана не трогать
#: чужой stdin.
q() {
