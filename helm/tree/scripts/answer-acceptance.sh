...
def named_entries(rows):
    """Имена из поимённой строки ответа.

    Прошлая версия искала «Фамилия И. О. —» и потому НЕ ВИДЕЛА врача без
    специальности и без даты: у него нет тире, строка выглядит как
    «Фамилия Имя Отчество.». Прогон 300 из-за этого выдал «имён 0» там,
    где врач был назван, и провалил гейт по вине измерителя, а не
    системы. Теперь разбирается сама поимённая строка.
    """
    for row in rows:
        if re.match(r"^(Есть|Ещё|Всего|Не нашёл|В данных)\b", row):
            continue
        if re.fullmatch(r"[А-ЯЁ][а-яё-]+(, [а-яё-]+)*\.", row):
            continue
        parts = [p.strip().rstrip(".") for p in row.split(";") if p.strip()]
        if parts and re.match(r"^[А-ЯЁ][а-яё]", parts[0]):
            return [p.split(" — ")[0].strip() for p in parts]
    return []


names = named_entries(lines)
...
if tag == "C1":
    ...
    # Гейт подключения и гейт покрытия разделены — ровно та же граница,
    # которую владелец провёл для четвёртого вызова. «Специальности
    # впереди» невыполнимо, если в корпусе не доказано ни одной
    # специальности: это состояние знания, а не поведение роутера.
    gates = {
    ...
    print("— покрытие специальностей (долг, не гейт подключения) —")
    print(("ЕСТЬ    " if specialty_head else "НЕТ     ")
          + "хотя бы одна доказанная специальность впереди ответа")
elif tag == "C2":
    gates = {
