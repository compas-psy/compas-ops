...
sudo docker compose exec -T n8n sh -lc 'wget -qO- --timeout=10 http://127.0.0.1:5678/healthz 2>&1 | head -c 200' 2>&1 | tail -2
echo
...
sudo docker compose exec -T forgejo sh -lc 'forgejo --version 2>&1 | head -1' 2>&1 | tail -1
sudo docker compose exec -T forgejo sh -lc 'forgejo admin user list 2>&1 | head -5' 2>&1 | tail -5
...
sudo docker compose exec -T forgejo sh -lc 'ls -1 /data/git/repositories 2>/dev/null | head -10' 2>&1 | sed 's/^/  /' | tail -11
